...
function insert_pilote(PDO $pdo, array $data): array
{
    ...
    try {
        ...
        try {
            $stmt = $pdo->prepare(
                'INSERT INTO pilotes (prenom, nom, photo) '
                . 'VALUES (?, ?, ?)'
            );
            ...
            // si la colonne photo n'existe pas ou requête invalide, retomber sur l'ancien schéma
                $stmt = $pdo->prepare(
                    'INSERT INTO pilotes (prenom, nom) VALUES (?, ?)'
                );
...
function insert_ecurie(PDO $pdo, array $data): array
{
    ...
    try {
        ...
        try {
                $stmt = $pdo->prepare(
                    'INSERT INTO ecuries (nom_ecuries, siege) VALUES (?, ?)'
                );
...
function insert_participation(PDO $pdo, int $pilote_id, int $ecurie_id, int $annee): array
{
    ...
    try {
        // doublon
        $checkSql = 'SELECT COUNT(*) FROM participations '
            . 'WHERE pilote_id = ? AND ecurie_id = ? AND annee = ?';
