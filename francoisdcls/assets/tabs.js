...
(function(){
  ...
  const fetchUrl = (base || '') + pathPart;
        try {
          ...
          try { console.debug('tabs: fetching', { fetchUrl: fetchUrl, href: href, BASE_PATH: window.BASE_PATH }); } catch(e){}
          if (debugBox) debugBox.textContent = 'fetching: ' + fetchUrl + '\nhref: ' + href + '\nBASE_PATH: ' + (window.BASE_PATH || '');
          const r = await fetch(fetchUrl, {credentials:'same-origin'});
            if (!r.ok) {
              ...
              if (debugBox) debugBox.textContent = 'failed: ' + r.status + '\n' + body.slice(0,2000);
              ...
              try { window.location.assign(fetchUrl); } catch(e) { /* ignore */ }
              return;
            }
            const html = await r.text();
          if (debugBox) debugBox.textContent = 'ok: ' + fetchUrl + '\nlen=' + html.length;
          ...
          if (debugBox) debugBox.textContent = 'exception: ' + String(err);
    ...
    window.addEventListener('popstate', ()=>{
      ...
      if (target) {
        ...
        if (typeof target._handleClick === 'function') {
          try { target._handleClick(null, { skipPush: true }); } catch(e) { console.error(e); }
        } else {
          target.click();
        }
      } else {
