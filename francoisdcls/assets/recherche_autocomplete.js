...
(function () {
  ...
  input.addEventListener('input', function () {
    ...
    if (q.length > 80) q = q.substr(0, 80);
    timer = setTimeout(function () {
      ...
      (window.siteFunctions && typeof window.siteFunctions.fetchJson === 'function' ? window.siteFunctions.fetchJson(url) : fetch(url).then(function (r) { return r.json(); }))
      .then(function (data) {
        clearSuggestions();
        ...
        data.forEach(function (item, idx) {
          ...
          if (item.type === 'pilote') {
            ...
            div.addEventListener('click', function () { window.location.href = window.BASE_PATH + '/pages/fiche_pilote.php?id=' + item.pilote_id; });
          } else if (item.type === 'ecurie') {
            div.textContent = item.ecurie_nom || 'Écurie';
            div.dataset.type = 'ecurie';
            div.dataset.ecurieId = item.ecurie_id || '';
            div.addEventListener('click', function () { window.location.href = window.BASE_PATH + '/pages/fiche_ecurie.php?id=' + item.ecurie_id; });
          }
      ...
      .catch(function (err) { console.error('Autocomplete fetch failed', err); renderEmpty(); });
    }, 180);
  });
})();
