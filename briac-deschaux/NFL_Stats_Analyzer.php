...
<div class="container">
    ...
    <?php if ($page === 'joueurs') : ?>
        ...
        <div class="card">
            ...
            <form method="post" action="services/add_player.php">
                ...
                <select name="id_team" required>
                    ...
                    <?php
                    $teams = $pdo->query("SELECT id_team, nom_team, logo_url FROM team ORDER BY nom_team")->fetchAll();
                    foreach ($teams as $t) {
        ...
        <div class="grid">
            <?php
            $stmt = $pdo->query("SELECT p.*, t.nom_team, t.logo_url 
                                 FROM player p 
            ...
            while ($pl = $stmt->fetch()) {
                ...
                echo "<div class='card'>
                        ...
                        <p><strong>Équipe:</strong> <img src='{$pl['logo_url']}' alt='' style='width:20px;height:20px;vertical-align:middle;'> {$pl['nom_team']}</p>
                        <p>Âge: {$pl['age']} ans</p>
